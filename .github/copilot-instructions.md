...
- Source is ESM (`type: module`) with explicit `.js` import extensions. `dist/` is generated by Rolldown — never edit it by hand. Browser artifacts target ES2018; UMD remains classic-script/AMD compatible and exposes `axios`, but v2 does not promise ES5 syntax or runtime polyfills. Keep `index.d.ts` (ESM) and `index.d.cts` (CJS) in sync for any public API change.
