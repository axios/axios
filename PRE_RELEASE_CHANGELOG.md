...
- **Typed request params:** Added a second, defaulted generic parameter to `AxiosRequestConfig` and propagated it through params serializers, internal configs, default response shapes, errors, cancellation guards (`CanceledError`/`isCancel`), request methods, callable instances, and `mergeConfig()` across ESM and CommonJS declarations. Default request results retain the request-data and params types on `response.config`, while explicit custom response types and the existing generic argument positions remain compatible. Untyped call sites retain the previous behavior through the `P = any` default. (**#11081**, closes **#4954**)
