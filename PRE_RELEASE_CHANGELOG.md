...
- **NO_PROXY wildcard matching:** `shouldBypassProxy` now honors `*` when it appears as one entry in a comma- or space-separated `NO_PROXY` list, not only when the entire variable is exactly `*`.
