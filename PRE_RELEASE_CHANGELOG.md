...
- **Interceptor storage:** Restored the v1.19.0 tolerance for a falsy `interceptors.*.handlers` array — `forEach` (and therefore every request) no longer throws when the array has been set to a falsy value, and `eject(String(id))` again ejects the interceptor. Both regressions came from the interceptor-storage rework behind **#11087**. (**#11114**)
