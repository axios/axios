...
- **NO_PROXY IPv4 normalization:** `shouldBypassProxy` now canonicalises NO_PROXY entries and request hostnames written in Node's URL-parser-accepted shorthand, octal (`0177.0.0.1`), and hex (`0x7f.0.0.1`) forms to dotted-decimal before comparison, so a `NO_PROXY=127.1` entry matches a request to `http://127.0.0.1/` (and vice versa). The helper is fail-safe: invalid octets (out of 0-255, non-hex digits), 1-part inputs, and 5+ parts are returned unchanged so the policy falls through to non-bypass. (**#11029**)
