...
- **Interceptor storage — nullish handlers:** Restored the v1.19.0 tolerance for a falsy `interceptors.request.handlers` or `interceptors.response.handlers` array. Interceptor iteration, ejection, and compaction now skip a nullish handlers list instead of throwing `Cannot read properties of null (reading 'length')` on every subsequent request. `eject()` also accepts an index-like string ID again, matching the array-index coercion that applied before the handler registry became a Map. (closes **#11114**)
