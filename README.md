...
> ⚠️ **Security note:**
> The `beforeRedirect` hook runs *after* sensitive headers (like `Authorization`) may have been stripped during redirects for security reasons (e.g., HTTPS → HTTP or cross-domain redirects).
>
> Re-injecting credentials without validating the protocol or destination can expose sensitive data over insecure connections.
>
> Always ensure that credentials are only added for trusted and secure (HTTPS) destinations.
Avoid re-adding credentials on downgraded (HTTPS → HTTP) redirects.
