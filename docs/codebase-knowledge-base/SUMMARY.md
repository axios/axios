...
| Data and state | No durable application storage. Request/config/response/error data lives in memory, streams, or platform objects; bounded module caches/pools support Fetch, proxies, and HTTP/2. |
...
- **Recovery:** This KB is documentation-only. Remove `docs/codebase-knowledge-base/` to reverse it, or refresh the pages and rerun the skill's stamp/validate/status commands after source changes.
