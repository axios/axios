...
The source defaults to the Node platform. Browser and React Native builds replace Node-only modules through package mappings. The package currently exposes ESM and generated CommonJS/browser bundles; the stated v2 direction is Node 20+ and ESM-only, but that future package shape is not yet implemented in this snapshot.
...
| Node HTTP adapter | Orchestrate HTTP/1, HTTP/2, redirects, proxies, agents, sockets, streams, compression, limits, and progress | `lib/adapters/http.js` | Node core, `follow-redirects`, `form-data`, `https-proxy-agent`, `proxy-from-env` | Request/stream/socket lifecycle and a module-level HTTP/2 session pool |
| Browser adapters | Dispatch through XHR or Fetch with credentials, progress, cancellation, and browser response semantics | `lib/adapters/xhr.js`, `lib/adapters/fetch.js` | Browser APIs or injected Fetch classes/functions | Per-request browser objects and Fetch factory cache |
...
| Generic helpers | URL, form, proxy, stream, signal, and normalization utilities reusable outside the request lifecycle | `lib/helpers/`, `lib/utils.js` | Core error/header types only where necessary | Mostly function-local state; selected bounded caches |
...
- Current package/build/CI still retain CJS and Node 12-18 compatibility, while the stakeholder-supplied v2 target is ESM-only on Node 20+. Treat the latter as future direction until manifests, build outputs, workflows, and docs change together.
...
| Environment selection | `package.json:12-63`; `lib/platform/index.js`; `lib/adapters/adapters.js:65-115` | Confirmed | Current package shape does not yet reflect the planned v2 baseline |
...
| Stateful resources | `lib/helpers/Http2Sessions.js`; adapter factory caches | Confirmed | No application database or durable business state exists |
