...
The Node 20+ half of the v2 direction is implemented but not yet completion-gated. PR #11161 passed build, lockfile, browser/unit, Bun/Deno, all ESM module/smoke lanes, all CJS smoke lanes, and CJS module lanes on Node 20, 22, and 24. Its original Node 26 CJS module lane failed before tests because Mocha 9.2.2's `yargs` 16 bridge is incompatible with Node 26 module loading; the isolated fixture now uses the repository's Mocha 11.8.0 baseline and Node 20+ cleanup APIs and passes locally on Node 20, 24, and 26, with the remote rerun pending. Per the maintainer's repository-wide directive on 2026-08-20, Axios does not use E2E test plans; plan close-out uses the repository-native checks and CI/runtime evidence defined by the implementation plan. ESM-only is explicitly separate; CJS outputs and declarations remain configured and tested until that coordinated migration updates exports, declarations, build outputs, smoke/module suites, CI, documentation, and release process.
...
| v2 compatibility      | `docs/plans/001-node-20-runtime-baseline.md`; `package.json`; `rollup.config.js`; PR #11161 CI | Node 20+ implemented on the PR branch; ESM-only remains intent | Corrected Node 26 CJS module lane still needs a green remote rerun                  |
