...
| TypeScript declarations | Root TypeScript 5.9 tooling; ESM fixture TS 5.9 | Public type surface and compile-time compatibility | `index.d.ts`; `index.d.cts`; `package.json:185`; `tests/module/esm/package.json:13-16` |
| Node.js | Current ESM CI: 20, 22, 24, 26; CJS CI: 12, 14, 16, 18 | Build/test runtime and Node HTTP transport | `.github/workflows/run-ci.yml:63-217` |
...
The stakeholder-supplied v2 target is Node 20+ and ESM-only. This is future direction; the current manifest has no `engines` constraint and still publishes/tests CJS.
...
- TypeScript module fixtures validate ESM against Node 20 definitions and CJS against Node 12 definitions in the current v1-shaped suite.
...
| Package outputs | `package.json:5-63,102-117`; `rollup.config.js:83-140` | Confirmed | Future v2 ESM-only output is not implemented |
| Runtime matrices | `.github/workflows/run-ci.yml:63-284` | Confirmed | No root `engines` declaration |
