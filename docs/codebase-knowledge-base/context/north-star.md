...
| Axios v2 modern baseline          | Node 20+ first, then a separately approved ESM-only distribution; v1.x retains compatibility     | Maintainer direction supplied 2026-08-19; `docs/plans/001-node-20-runtime-baseline.md`; current manifests/workflows | Node 20+ implemented but completion-blocked; ESM-only remains intent |
...
| v2 Node/ESM direction   | Maintainer directives supplied 2026-08-19 and 2026-08-20; Plan 001; current manifests, Rollup config, and CI workflows | Node 20+ confirmed in dirty implementation; ESM-only remains intent | Repository-native Plan 001 validation still lacks complete runtime CI results |
