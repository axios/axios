...
| Axios v2 modern baseline          | Node 20+ first, then a separately approved ESM-only distribution; v1.x retains compatibility     | Maintainer directives supplied 2026-08-20; `docs/plans/001-node-20-runtime-baseline.md`; current manifests/workflows | Node 20+ implemented but completion-blocked; ESM-only remains intent |
...
| v2 Node/ESM direction   | Maintainer directives supplied 2026-08-20; Plan 001; current manifests, Rollup config, and CI workflows | Node 20+ confirmed on PR #11161; ESM-only remains intent | Corrected Node 26 CJS module lane still needs a green remote rerun           |
