...
The stakeholder direction supplied with this task is: **Axios v2 may drop non-ESM support and target Node.js 20+, while v1.x continues legacy runtime/module support.** The Node 20+ portion is now encoded in the v2 branch and completed under archived Plan 001; the ESM-only portion remains an unimplemented, separately gated migration.
...
| Axios v2 modern baseline          | Node 20+ first, then a separately approved ESM-only distribution; v1.x retains compatibility     | Maintainer directives supplied 2026-08-20; `docs/plans/archive/2026-08-21-001-node-20-runtime-baseline.md`; current manifests/workflows | Node 20+ complete and matrix-verified; ESM-only remains intent |
...
The repository now records the sequencing and completion criteria for the Node 20+ first step in Plan 001, including retention of CJS and Bun/Deno verification. The ESM-only package shape, declaration changes, exact release mechanics, and broader migration criteria remain unresolved follow-up work.
...
- Backward compatibility currently retains deprecated `CancelToken`, `Cancel`, `axios.all`, and CommonJS artifacts; Node tests now target the v2 floor of Node 20 and newer.
...
- A completed ESM-only plan defining Rollup outputs, package exports, declarations, smoke/module matrices, and public migration documentation.
...
| v2 Node/ESM direction   | Maintainer directives supplied 2026-08-20; archived Plan 001; current manifests, Rollup config, and CI workflows | Node 20+ complete and matrix-verified on PR #11161; ESM-only remains intent | ESM-only scope and v2 release mechanics still require separate plans         |
