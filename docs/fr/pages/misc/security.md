...
| --- | --- | --- |
| [`baseURL`](/pages/advanced/request-config#baseurl) | Certaines applications traitent un préfixe de chemin dans `baseURL`, comme `https://api.example.com/v1/`, comme une limite de requête. Des valeurs relatives de `url` contrôlées par l'utilisateur peuvent contenir des segments `..` que le parseur final d'URL normalise et résout en dehors de ce préfixe de chemin. | Ne vous appuyez pas sur `baseURL` pour isoler les chemins. Validez les chemins non fiables avant de les transmettre à axios ; rejetez les URLs absolues ou relatives au protocole et les segments `..`, ou vérifiez l'origine et le pathname de l'URL résolue avec une liste d'autorisation. |
