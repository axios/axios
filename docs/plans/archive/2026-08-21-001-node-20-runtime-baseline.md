...
| Last updated    | 2026-08-21T15:07:05+02:00                                                                                                                                                                              |
...
| 5    | The packed v2 artifact and every affected compatibility path are verified            | Steps 1–4    | Sequential                               | Complete |
...
- 2026-08-21T11:44:51+02:00 — Corrected-fixture [CI run `32468769721`](https://github.com/axios/axios/actions/runs/32468769721) passed every ESM/CJS smoke and module lane on Node 20, 22, 24, and 26 plus Bun and Deno, including [Node 22 CJS module job `96731529782`](https://github.com/axios/axios/actions/runs/32468769721/job/96731529782) and the previously failing [Node 26 job `96731529797`](https://github.com/axios/axios/actions/runs/32468769721/job/96731529797). Its [Build and run vitest job `96730941588`](https://github.com/axios/axios/actions/runs/32468769721/job/96730941588) separately records successful install, lint, build, unit/browser, package, dependency-review, and artifact-upload steps.
- 2026-08-21T13:23:30+02:00 — Independent checks on the same `37ba561e7f121b0f442c6a6bd4975c62c20aa5a9` revision passed in [Lockfile lint run `32468769979`](https://github.com/axios/axios/actions/runs/32468769979), [reproducibility run `32468769704`](https://github.com/axios/axios/actions/runs/32468769704), [Bundle Size run `32468769759`](https://github.com/axios/axios/actions/runs/32468769759), and [zizmor run `32468769694`](https://github.com/axios/axios/actions/runs/32468769694). This explicit evidence satisfies both remaining success criteria without attributing independent workflows to the CI run.
- 2026-08-21T15:00:28+02:00 — The repository-native completion audit passed at final PR revision `09b51559f573fc10151e25c9f65f5d65aaa4a896`. Current [CI run `32477064358`](https://github.com/axios/axios/actions/runs/32477064358) passed build, lint, dependency review, unit/browser, packaging, Bun/Deno, and every ESM/CJS module and smoke lane on Node 20, 22, 24, and 26. Same-revision [lockfile run `32477064428`](https://github.com/axios/axios/actions/runs/32477064428), [reproducibility run `32477064502`](https://github.com/axios/axios/actions/runs/32477064502), [bundle-size run `32477064362`](https://github.com/axios/axios/actions/runs/32477064362), and [zizmor run `32477064291`](https://github.com/axios/axios/actions/runs/32477064291) also passed. All PR review threads are resolved. The branch matched its remote and the worktree was clean before this documentation-only close-out.
- 2026-08-21T15:07:05+02:00 — PR #11161 merged into `v2.x` as `9e51e031b453d7b766eaa9410b8b338f811e2cbf`. The archive-only follow-up branch starts from that merge revision, so its diff contains only plan close-out and knowledge-base documentation.
...
Plan 001 is complete on PR #11161 at final implementation revision `09b51559f573fc10151e25c9f65f5d65aaa4a896`, merged into `v2.x` as `9e51e031b453d7b766eaa9410b8b338f811e2cbf`. Axios v2 now declares Node `>=20.0.0` across the published and Node-based private packages, targets the temporary Node CommonJS build and TypeScript fixture at Node 20, and validates the retained ESM/CJS package shapes on Node 20, 22, 24, and 26. The staged contributor and pre-release guidance directs older-Node consumers to v1.x and leaves ESM-only removal to a separate approved migration.

Repository-native validation was selected under the maintainer's 2026-08-20 repository-wide directive, “in this repo we will never have e2e tests.” No manual checks are required by this plan. The final PR revision passes the complete package/runtime matrix, build, lint, dependency review, unit/browser, lockfile, reproducibility, bundle-size, workflow-security, Bun, and Deno checks; all review threads are resolved.

The completion audit started from a clean `feat/node-20-runtime-baseline` worktree synchronized with its remote. Archiving this plan and refreshing `docs/codebase-knowledge-base/` create only local documentation changes atop that revision. There is no data migration, external-state mutation, feature flag, or runtime rollback action in the close-out. Before v2 publication the implementation can be reverted as a Git change; after publication, consumers that cannot run Node 20+ must remain on v1.x. The ESM-only package migration, v2 release automation, and staged public documentation remain separately approved follow-ups.
