  ...
  return async (config) => {
    ...
    try {
      ...
      if (pendingResponseError) {
        request && !pendingResponseError.request && (pendingResponseError.request = request);
      ...
      if (err && err.cause instanceof AxiosError) {
        request && !err.cause.request && (err.cause.request = request);
        throw err.cause;
      }
