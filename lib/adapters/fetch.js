...
function isResponseEligibleForBody(response) {
  // status cannot be present in `knownNoBodyResponseStatuses` in order to have a parse-able body
...
function processResponseBody(config, response, responseHeaders) {
  ...
  if (hasContentType && (responseHeaders.has(contentLengthHeader) || responseHeaders.has(transferEncodingHeader))) {
    ...
    handler = selectHandlerFromConfig(config, handlers);
    if (handler == null) {
...
function dispatchFetch(config, resolve, reject) {
  ...
  const handleResponse = response => {
    ...
    if (eligibleForBody) {
      let [responseHasBody, responseHandler] = processResponseBody(
          config,
          response,
          responseHeaders,
      )
