...
const factory = (env) => {
  ...
  // Cloudflare Workers throws for any cache mode other than 'no-store' and 'no-cache'
  // see https://github.com/cloudflare/workerd/issues/698. The second probe keeps the
  // explicit default whenever a Request cannot be built for some unrelated reason.
  const supportsDefaultCacheMode =
    isRequestSupported &&
    (test(() => new Request(platform.origin, { cache: 'default' })) ||
      !test(() => new Request(platform.origin, { cache: 'no-store' })));

  const requestDefaults = Object.assign({}, DEFAULT_REQUEST_OPTIONS);

  if (!supportsDefaultCacheMode) {
