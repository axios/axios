...
const factory = (env) => {
  ...
  // Only back-fill defaults the host runtime actually accepts; a value that is valid per
  // spec can still be rejected by the runtime's Request constructor (see workerd#902).
  const supportedRequestOptions =
    isRequestSupported &&
    (() => {
      const supported = Object.create(null);

      utils.forEach(DEFAULT_REQUEST_OPTIONS, (value, key) => {
        test(() => new Request(platform.origin, { [key]: value })) && (supported[key] = value);
      });
  ...
  return async (config) => {
    ...
    try {
      ...
      if (isRequestSupported) {
        // Caller-supplied fetchOptions always win; unset keys fall back only to defaults
        // this runtime was probed to support.
        utils.forEach(supportedRequestOptions, (value, key) => {
