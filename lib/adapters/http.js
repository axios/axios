      ...
      function abort(reason) {
        try {
          if (!reason || reason.type) {
            const signalReason = config.signal ? config.signal.reason : undefined;
            abortEmitter.emit('abort', new CanceledError(null, config, req, signalReason));
          } else {
            abortEmitter.emit('abort', reason);
          }
        } catch (err) {
