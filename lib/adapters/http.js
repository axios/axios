  ...
  function httpAdapter(config) {
    return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
      ...
      if (Number.isNaN(httpVersion)) {
        throw new AxiosError(`Invalid protocol version: '${config.httpVersion}' is not a number`, AxiosError.ERR_BAD_OPTION_VALUE);
      }

      if (httpVersion !== 1 && httpVersion !== 2) {
        throw new AxiosError(`Unsupported protocol version '${httpVersion}'`, AxiosError.ERR_BAD_OPTION_VALUE);
      }
