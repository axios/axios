  ...
  function httpAdapter(config) {
    return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
      ...
      onDone((response, isRejected) => {
        ...
        } else if (responseType === 'ndjson' && data && typeof data[Symbol.asyncIterator] === 'function') {
          // The NDJSON generator releases the request lifecycle in its finally block.
        } else {
          onFinished();
        }
      });
      ...
      if (protocol === 'data:') {
        ...
        } else if (responseType === 'ndjson') {
          const source = stream.Readable.from(convertedData);
          convertedData = (async function*() {
            try {
              yield* fromNdjson(source);
            } catch (err) {
              throw err instanceof AxiosError
                ? err
                : AxiosError.from(err, AxiosError.ERR_BAD_RESPONSE, config);
            } finally {
              onFinished();
            }
          })();
      ...
      req = transport.request(options, function handleResponse(res) {
        ...
        const enforceMaxContentLength = (source) => {
          if (maxContentLength <= -1) {
            return source;
          }
          ...
          return stream.Readable.from(limitedSource(), {objectMode: false});
        };

        if (responseType === 'ndjson') {
          const source = enforceMaxContentLength(responseStream);
          response.data = (async function*() {
            try {
              ...
              throw err instanceof AxiosError
                ? err
                : AxiosError.from(err, AxiosError.ERR_BAD_RESPONSE, config, lastRequest, response);
            } finally {
              onFinished();
            }
          })();
          ...
          response.data = enforceMaxContentLength(responseStream);
          settle(resolve, reject, response);
        } else {
