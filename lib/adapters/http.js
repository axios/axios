...
export default isHttpAdapterSupported &&
  function httpAdapter(config) {
    return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
      ...
      // Node rejects negative socket timeouts with a RangeError, which follow-redirects
      // raises outside the request promise and would crash the process. parseFloat keeps
      // fractional values like -0.5 negative, where parseInt would truncate them to -0.
      if (own('timeout') && parseFloat(own('timeout')) < 0) {
        throw new AxiosError(
          '`config.timeout` must be a non-negative number',
          AxiosError.ERR_BAD_OPTION_VALUE,
          config
        );
      }
