...
function setProxy(options, configProxy, location, isRedirect, configHttpsAgent) {
  ...
  // Strip any tunneling agent installed for the previous hop so a redirect that
  // drops the proxy or crosses an HTTPS↔HTTP boundary doesn't reuse a stale one.
  if (isRedirect && options.agent instanceof HttpsProxyAgent) {
  ...
  if (proxy) {
    ...
    if (targetIsHttps) {
      ...
      // behavior already promised by THREATMODEL.md (T-R9). Skip when the user
      // supplied their own httpsAgent; they own tunneling. HttpsProxyAgent puts
      // Proxy-Authorization on the CONNECT request only, never on the wrapped
      // TLS request, which is why we don't stamp it onto options.headers here.
      if (!configHttpsAgent) {
        ...
        const proxyURL = new URL(
          `${normalizedProtocol}//${proxyHost}${proxyPort ? ':' + proxyPort : ''}`
        );
        if (proxyAuth && typeof proxyAuth === 'string') {
          const colonIdx = proxyAuth.indexOf(':');
          proxyURL.username = colonIdx >= 0 ? proxyAuth.slice(0, colonIdx) : proxyAuth;
          proxyURL.password = colonIdx >= 0 ? proxyAuth.slice(colonIdx + 1) : '';
        }
