...
const http2Transport = {
  request(options, cb) {
    ...
    const { http2Options, headers, auth } = options;
...
export default isHttpAdapterSupported &&
  function httpAdapter(config) {
    return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
      ...
      const { useAxiosRedirects } = config.transitional || {};
      ...
      if (utils.isSpecCompliantForm(data)) {
        ...
        } else if(utils.isAsyncIterable(data) && stream.Readable.from) {
      ...
      const contentLength = utils.toFinite(headers.getContentLength());
        ...
        data = stream.pipeline(
          [
            ...
            new AxiosTransformStream({
              maxRate: utils.toFinite(maxUploadRate),
            }),
          ],
          utils.noop
        );
      ...
      if (!auth && (parsed.username || parsed.password)) {
        const urlUsername = utils.decodeURIComponentSafe(parsed.username);
        const urlPassword = utils.decodeURIComponentSafe(parsed.password);
      ...
      req = transport.request(options, function handleResponse(res) {
        ...
        const responseLength = utils.toFinite(res.headers['content-length']);

        if (onDownloadProgress || maxDownloadRate) {
          const transformStream = new AxiosTransformStream({
            maxRate: utils.toFinite(maxDownloadRate),
          });
