...
function setProxy(options, configProxy, location, isRedirect) {
  ...
  if (proxy) {
    ...
    if (proxyAuth) {
      ...
      const authIsObject = proxyAuth !== null && typeof proxyAuth === 'object';
      const authUsername =
  ...
  function httpAdapter(config) {
    return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
      ...
      if (utils.isSpecCompliantForm(data)) {
        ...
        // Only copy headers safe for boundary/length signalling. Mirror the whitelist
        // applied in resolveConfig.js so a custom getHeaders() cannot inject arbitrary
        // headers (e.g. X-Forwarded-For, Host) into the outbound request.
        const formHeaders = data.getHeaders();
        const allowedFormHeaders = ['content-type', 'content-length'];
        Object.entries(formHeaders).forEach(([key, val]) => {
          if (allowedFormHeaders.includes(key.toLowerCase())) {
            headers.set(key, val);
          }
        });
