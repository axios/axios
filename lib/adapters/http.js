...
var getDirectAgent = require('../helpers/getDirectAgent');
...
function dispatchBeforeRedirect(options, responseDetails, requestDetails) {
  ...
  try {
    sameOrigin = new url.URL(requestDetails.url).origin === new url.URL(options.href).origin;
  } catch (err) {
    // Leave the comparison false when either URL is unavailable.
  }
  if (!sameOrigin) {
    Object.keys(options.headers).forEach(function(header) {
      if (/^(authorization|cookie)$/i.test(header)) {
...
function setDirectAgent(options, httpAgent, httpsAgent) {
  var secure = isHttps.test(options.protocol);
  var agent = getDirectAgent(secure ? httpsAgent : httpAgent, secure ? https : http);
  options.agent = agent;
...
module.exports = function httpAdapter(config) {
  return new Promise(function dispatchHttpRequest(
    ...
    if (protocol === 'data:') {
      ...
      if (config.maxContentLength > -1 &&
        (responseType === 'blob' ? convertedData.size : convertedData.length) > config.maxContentLength) {
        return reject(
          new AxiosError(
    ...
    var httpAgent = config.httpAgent;
    var httpsAgent = config.httpsAgent;
    var options = {
      path: buildURL(
        parsed.path,
        config.params,
    ...
    if (configProxy === false) {
      setDirectAgent(options, httpAgent, httpsAgent);
      options.beforeRedirects.direct = function(redirectOptions) {
