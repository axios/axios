...
// Module-scope socket 'error' handler (`this` is the socket): keeping it out of
// the adapter scope means it captures no per-request closure context. An inline
// closure here is installed once per socket and never removed, so it would pin
// the installing request's entire adapter scope (settled response promise,
// response body, boundSockets) for the lifetime of a pooled keep-alive socket.
  ...
  function httpAdapter(config) {
    return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
      ...
      req.on('socket', function handleRequestSocket(socket) {
        ...
        // symbol, which is swapped as the socket is reassigned. It must be a
        // module-scope function: an inline closure would retain the installing
        // request's context (response included) for the socket's whole lifetime.
