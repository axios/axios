...
function sanitizeNodeHeaderValue(value) {
  if (utils.isArray(value)) {
    return value.map(sanitizeNodeHeaderValue);
  }
...
function toNodeRequestHeaders(headers) {
  const nodeHeaders = Object.create(null);

  utils.forEach(headers.toJSON(), (value, header) => {
    nodeHeaders[header] = sanitizeNodeHeaderValue(value);
  });
