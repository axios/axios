...
export default isHttpAdapterSupported &&
  function httpAdapter(config) {
    return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
      ...
      // Track the most recently bound socket so a single 'close' listener on the
      // outer RedirectableRequest can release ownership without stacking a new
      // listener per redirect hop. follow-redirects re-emits the 'socket' event
      // for each hop's native request onto the same outer request, so attaching
      // per-request listeners inside this handler would accumulate across hops
      // and trigger MaxListenersExceededWarning at >= 11 redirects.
      let lastBoundSocket = null;
      ...
      req.on('socket', function handleRequestSocket(socket) {
        ...
        lastBoundSocket = socket;
      });

      req.once('close', function clearCurrentReq() {
        if (lastBoundSocket && lastBoundSocket[kAxiosCurrentReq] === req) {
          lastBoundSocket[kAxiosCurrentReq] = null;
        }
      });
