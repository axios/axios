...
const readXsrfCookie = (resolvedConfig, config, name) => {
  const { cancelToken, signal } = resolvedConfig;

  if (!cancelToken && !signal) {
    return cookies.readAsync(name);
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const cleanup = () => {
      cancelToken && cancelToken.unsubscribe(onCanceled);
      signal && signal.removeEventListener('abort', onCanceled);
    };
    ...
    cancelToken && cancelToken.subscribe(onCanceled);
    if (settled) return;
...
export default isXHRAdapterSupported &&
  async function (config) {
    const useAsyncXsrf =
      platform.hasStandardBrowserEnv && typeof window !== 'undefined' && window.cookieStore;
    let xsrfCookieName;
    ...
    if (xsrfCookieName) {
      const xsrfValue = await readXsrfCookie(_config, config, xsrfCookieName);

      if (xsrfValue) {
        _config.headers.set(_config.xsrfHeaderName, xsrfValue);
      }
    }
