...
class Axios {
  ...
  _request(configOrUrl, config) {
    ...
    // Flatten headers. A method or `common` key only carries structural
    // defaults when it holds a plain object or an AxiosHeaders instance; any
    // other value under one of those names, eg the `Link` request header, is a
    // header to send.
    const isHeadersBucket = (value) =>
      value == null || utils.isPlainObject(value) || value instanceof AxiosHeaders;

    let contextHeaders =
      headers &&
      utils.merge(
        isHeadersBucket(headers.common) ? headers.common : undefined,
        isHeadersBucket(headers[config.method]) ? headers[config.method] : undefined
      );

    headers &&
      utils.forEach(methodList.concat('common'), (method) => {
        if (isHeadersBucket(headers[method])) {
