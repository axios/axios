...
class Axios {
  ...
  _request(configOrUrl, config) {
    ...
    if (typeof configOrUrl === 'string') {
      // Flatten through the same safe-materialization helper used at other config
      // boundaries (see dispatchRequest.js) so a caller-owned config with unsafe
      // own keys (__proto__/constructor/prototype) isn't read while copying it.
      config = { ...utils.toSafeFlatObject(config), url: configOrUrl };
    } else {
      config = configOrUrl || {};
    }
