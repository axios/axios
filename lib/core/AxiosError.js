...
function serializeConfigValue(value, keys, key) {
  var result;
  ...
  if (utils.isArray(value)) {
    ...
    utils.forEach(value, function eachArrayValue(item, index) {
      result[index] = serializeConfigValue(item, keys, index);
    });
    return result;
  }
  ...
  if (utils.isPlainObject(value)) {
    ...
    utils.forEach(value, function eachObjectValue(item, itemKey) {
      result[itemKey] = serializeConfigValue(item, keys, itemKey);
    });
    return result;
  }
...
function serializeConfig(config) {
  ...
  return serializeConfigValue(config, getRedactKeys(config));
}
