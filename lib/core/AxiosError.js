...
// Transport handles carry no diagnostic value once serialized, and their
// internals are heavily cross-referenced -- an agent points at its sockets and
// every socket points back -- so walking them from an error is both useless and
// expensive. Collect them by identity so they are labelled wherever they are
// reached, rather than only under the key they were configured on, and so the
// config itself is neither copied nor mutated.
const AGENT_KEYS = {
