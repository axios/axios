...
// expensive. Replace them with a label instead.
const AGENT_KEYS = {
...
function replaceAgents(config) {
  ...
  utils.forEach(AGENT_KEYS, (label, key) => {
    if (utils.isObject(config[key])) {
      copy = copy || Object.assign({}, config);
      copy[key] = label;
    }
  });
...
class AxiosError extends Error {
  ...
  toJSON() {
    ...
    const safeConfig = replaceAgents(config);
    const serializedConfig =
      utils.isArray(redactKeys) && redactKeys.length > 0
        ? redactConfig(safeConfig, redactKeys)
        : utils.toJSONObject(safeConfig);
