...
function replaceAgents(config) {
  ...
  AGENT_KEYS.forEach((key) => {
    ...
    // enough here. Read the descriptor, not `config[key]`: an inherited
    // accessor can throw or simply have no way to know its value is an
    // object without invoking it, so any accessor is treated as agent-like.
    const descriptor = getOwnOrPrototypeDescriptor(config, key);
    if (descriptor && ('get' in descriptor || utils.isObject(descriptor.value))) {
      ...
      // `defineProperty`, not assignment: the clone still inherits the same
      // prototype, so a getter-only accessor up the chain would make a plain
      // `result[key] = AGENT` throw in strict mode instead of shadowing it.
