...
function trimSPorHTAB(str) {
  ...
  while (start < end) {
    ...
    if (code !== HORIZONTAL_TAB && code !== SPACE) {
      break;
    }
...
function sanitizeHeaderValue(str) {
  ...
  for (const character of str) {
    ...
    // Keep HTAB and visible characters while excluding DEL and C1 controls.
    if (
      codePoint === HORIZONTAL_TAB ||
      (codePoint >= SPACE &&
        codePoint !== DELETE &&
  ...
  return trimSPorHTAB(sanitized.join(''));
}
