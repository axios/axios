...
function trimHandlers(handlers) {
  while (handlers && handlers.length && handlers[handlers.length - 1] === null) {
    handlers.pop();
  }
}
...
class InterceptorManager {
  ...
  eject(id) {
    ...
    const entry = internals.handlerEntries.get(Number(id));
  ...
  forEach(fn) {
    ...
    try {
      ...
      if (!--internals.iterationDepth) {
        syncHandlerEntries(this, internals);
        trimHandlers(this.handlers);
        internals.handlersLength = this.handlers ? this.handlers.length : 0;
      }
    }
  }
}
