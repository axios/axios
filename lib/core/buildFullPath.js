...
export default function buildFullPath(baseURL, requestedURL, allowAbsoluteUrls, config) {
  // Without a baseURL to resolve against, a nullish url cannot produce a target
  // at all; fail fast here so the browser adapters never request the current
  // page instead. A baseURL-only request stays supported via combineURLs.
  if (requestedURL == null && !baseURL) {
    throw new AxiosError(
      'Invalid URL: the request `url` is missing',
      AxiosError.ERR_INVALID_URL,
      config
    );
  }
