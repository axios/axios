...
export default function dispatchRequest(_config) {
  ...
  // The prototype is preserved so safe-flat configs keep their shape.
  function restoreRequestData(responseConfig) {
    return Object.assign(Object.create(Object.getPrototypeOf(responseConfig)), responseConfig, {
      data: originalData,
    });
  }
      ...
      if (!isCancel(reason)) {
        ...
        if (reason && reason.response) {
          if (reason.response.config) {
            reason.response.config = restoreRequestData(reason.response.config);
          }
