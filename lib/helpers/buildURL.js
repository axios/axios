...
export default function buildURL(url, params, options) {
  ...
  if (url === null) {
    ...
    normalizedUrl = url;
  } else if (typeof url === 'object' && url !== null && Object.prototype.hasOwnProperty.call(url, 'href') && typeof url.href === 'string') {
    normalizedUrl = url.href;
  } else if (params) {
    // Reject any other types (numbers, booleans, or objects with non-string href)
    // when params are present, to prevent silent coercion.
    throw new AxiosError('Invalid URL: url must be a string or URL object when params are provided', AxiosError.ERR_INVALID_URL);
  } else {
    // No params, no URL: just return empty or the value as string
