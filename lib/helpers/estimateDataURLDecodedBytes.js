...
const estimateDataURLBytes = (url, estimateBase64, base64Pattern) => {
  ...
  const isBase64 = base64Pattern.test(meta);
...
export function estimateDataURLBufferAllocation(url) {
  // fromDataURI recognizes only the lowercase protocol and encoding suffix.
  if (typeof url !== 'string' || url.slice(0, 5) !== 'data:') return 0;
  return estimateDataURLBytes(url, estimateBase64BufferAllocation, /;base64$/);
}
