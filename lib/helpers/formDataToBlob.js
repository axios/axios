...
export default function formDataToBlob(form, Blob, limit, limitError) {
  ...
  if (crypto && utils.isFunction(crypto.getRandomValues)) {
    ...
    for (let i = 0; i < random.length; i++) random[i] = Math.random() * 256;
  }
  const boundary =
  ...
  for (const [name, value] of form) {
    ...
    if (!Number.isFinite(valueLength) || valueLength < 0) {
      throw new AxiosError('Unsupported FormData part size', AxiosError.ERR_BAD_REQUEST);
    }
