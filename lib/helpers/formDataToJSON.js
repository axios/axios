    ...
    if (isLast) {
      if (utils.hasOwnProp(target, name)) {
        if (utils.isArray(target[name])) {
          // Append in place; `concat` copies the whole accumulated array on
          // every duplicate name, making M same-name fields O(M^2). The loop
          // preserves `concat`'s one-level flattening of an array `value`.
          if (utils.isArray(value)) {
            for (let i = 0; i < value.length; i++) {
              target[name].push(value[i]);
            }
          } else {
