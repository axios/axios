...
var domainToASCII = require('url').domainToASCII;
...
function normalizeNoProxyHost(hostname) {
  ...
  if (hostname.indexOf(':') !== -1) {
    try {
      hostname = new URL('http://[' + hostname + ']/').hostname.slice(1, -1);
    } catch (err) {
      return hostname;
    }
  } else if (/[\u0080-\uffff]/.test(hostname)) {
    var prefix = /^[*.]+/.exec(hostname);
    var start = prefix ? prefix[0] : '';
    var ascii = domainToASCII(hostname.slice(start.length));
    if (!ascii) return '';
    hostname = start + stripTrailingDots(ascii);
  }
...
module.exports = function shouldBypassProxy(location) {
  ...
  var noProxy = (process.env.npm_config_no_proxy || process.env.NPM_CONFIG_NO_PROXY ||
    process.env.no_proxy || process.env.NO_PROXY || '').toLowerCase();
