 ...
 * zero-prefixed multi-digit all-`0-7` parts, 10 otherwise. A part that fits
 * multiple bases (e.g. `08`, where `8` is not a valid octal digit) is parsed
 * as the longest-prefix match — so `08` falls through to decimal `8` while
 * `0177` is octal `127`. Per-part permissiveness matches Node's URL parser,
 * and the comparison layer falls through to non-bypass if either side rejects
 * the form (fail-safe).
 *
...
const normalizeIPAddress = (host) => {
  ...
  // Tail must be 1..tailSlots decimal digits — Node's shorthand expansion only
  // accepts decimal digits in the tail (no octal/hex), and accepting more here
  // would diverge from Node for the common case.
  if (!/^[0-9]+$/.test(tail) || tail.length < 1 || tail.length > tailSlots) {
    return host;
  }
  const padded = tail.padStart(tailSlots, '0');
  const tailOctets = [];
  for (const ch of padded) {
    tailOctets.push(parseInt(ch, 10));
  }
