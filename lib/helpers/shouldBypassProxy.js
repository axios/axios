...
// matching. 0.0.0.0 is covered by LOOPBACK_HOSTNAMES; this handles the IPv6
// forms (`::`, `0:0:0:0:0:0:0:0`) so both families bypass symmetrically.
const isIPv6Unspecified = (host) => {
  if (host === '::') return true;

  const groups = host.split(':');
  return groups.length === 8 && groups.every((group) => /^0+$/.test(group));
};
