 ...
 */
const toJSONObject = (obj, replacements) => {
  const visited = new WeakSet();

  const visit = (source) => {
    if (isObject(source)) {
      // Stand-ins are keyed by identity, so an object reached through any path
      // -- including an alias under another key -- resolves to the same label.
      if (replacements) {
