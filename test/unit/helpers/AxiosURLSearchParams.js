...
describe("helpers::AxiosURLSearchParams", function () {
  describe("null byte encoding", function () {
    it("should not reverse the safe percent-encoding of null bytes", function () {
      ...
      assert.strictEqual(
        serialized.indexOf("\x00"),
        -1,
        "serialized string must not contain a raw null byte",
      );
      assert.ok(
        /%00/.test(serialized),
        "null byte must remain percent-encoded as %00",
      );
    });
  });
});
