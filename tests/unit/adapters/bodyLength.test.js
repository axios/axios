...
import { estimateDataURLBufferAllocation as estimate } from '../../../lib/helpers/estimateDataURLDecodedBytes.js';
...
describe('encoded body length', function () {
  [
    ...
    [
      ...
      it(
        'limits ' + fixture.name + ' through the ' + transport.name + ' transport',
        async function () {
          await withUploadServer(async function (url, received) {
            await assert.rejects(
              axios.post(
                url,
                stream.Readable.from(fixture.chunks),
                Object.assign(
                  {
                    proxy: false,
                    maxBodyLength: 3,
            ...
            assert.ok(
              received.every(function (length) {
                return length <= 3;
              })
            );
          });
    ...
    it(
      'allows an exact byte limit through the ' + transport.name + ' transport',
      async function () {
        await withUploadServer(async function (url, received) {
          var response = await axios.post(
            url,
            stream.Readable.from(['é', 'é']),
            Object.assign(
              {
                proxy: false,
                maxBodyLength: 4,
  ...
  ['BASE64=x', 'base64=x', 'base64note=x'].forEach(function (parameter) {
    ...
    ['text', 'arraybuffer', 'stream', 'blob'].forEach(function (responseType) {
      it(
        'checks data URL bytes before returning ' + responseType + ' with ' + parameter,
        async function () {
          await assert.rejects(
            axios.get(url, {
              maxContentLength: 3,
  ...
  ['TQ', 'TWE', 'TWFu', 'TQ=='].forEach(function (encoded) {
    it('counts a base64 body of length ' + encoded.length, async function () {
      ...
      var response = await axios.get(url, {
        maxContentLength: expected.length,
        responseType: 'arraybuffer',
      });
      assert.deepStrictEqual(response.data, expected);
      await assert.rejects(
        axios.get(url, { maxContentLength: expected.length - 1 }),
        function (error) {
          return error.code === 'ERR_BAD_RESPONSE';
        }
      );
