...
const it = (name, testFn, timeout) => vitestIt(name, wrapDoneCallback(testFn), timeout);
it.skip = (name, testFn, timeout) => vitestIt.skip(name, wrapDoneCallback(testFn), timeout);
...
describe('supports http with nodejs', function () {
  ...
  it('should wrap HTTP errors and keep stack', async function () {
    if (nodeMajorVersion <= 12) {
      return; // node 12 support for async stack traces appears lacking
      return;
    }
  ...
  it('should support HTTPS proxies', function (done) {
    var options = {
      key: fs.readFileSync(path.join(__dirname, '../../../test/unit/adapters/key.pem')),
      cert: fs.readFileSync(path.join(__dirname, '../../../test/unit/adapters/cert.pem')),
    };
  ...
  describe('FormData', function () {
    describe('form-data instance (https://www.npmjs.com/package/form-data)', (done) => {
      it('should allow passing FormData', function (done) {
        ...
        const image = path.resolve(__dirname, '../../../test/unit/adapters/axios.png');
